      ...
      With TcxDBBotao(Sender).DataSource Do
         begin
            if (DataSet.FieldByName('status').AsString = '5') Or
               (DataSet.FieldByName('status').AsString = '9') then
               begin
                  Informacao('Envio não permitido.'#13'Documento fiscal ' + IfThen(DataSet.FieldByName('status').AsString = '5', 'autorizado', IfThen(DataSet.FieldByName('status').AsString = '9', 'cancelado')) + '.');
                  Exit;
               end;
            ...
            if Not dmPDV.IsTerminalFiscal then
               begin
                  Informacao('Este terminal não possui este recurso.');
                  Exit;
               end;
   ...
   Try
      With TcxDBBotao(Sender).DataSource Do
         begin
            ...
            if dlgEditaCampoMemo.Executa('.:: Informações Adicionais ::.', DataSet.FieldByName('informacoes_adicionais')) then
               begin
                  DataSet.Refresh;
               end;
         end;
...
begin
  inherited;
   ...
   Else if Sender.AsString = '2' then
           Text := 'Contingência com SCAN'
   Else if Sender.AsString = '3' then
           Text := 'Contingência via DPEC'
   Else if Sender.AsString = '4' then
           Text := 'Contingência FS-DA';
   DisplayText := Text <> '';
end;
...
begin
  inherited;
   if Text = 'Normal' then
      Sender.AsString := '0'
   Else if Text = 'Contingênca FS' then
           Sender.AsString := '1'
   Else if Text = 'Contingência com SCAN' then
           Sender.AsString := '2'
   Else if Text = 'Contingência via DPEC' then
           Sender.AsString := '3'
   Else if Text = 'Contingência FS-DA' then
           Sender.AsString := '4';
end;
